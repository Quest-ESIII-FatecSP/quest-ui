...
export class RoomComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private showModalRoleta() {
    try {
      ...
      const api = (window as any).roletaAPI;
      if (el && isIframe) {
        try { (el as HTMLIFrameElement).setAttribute('scrolling', 'no'); (el as any).style.overflow = 'hidden'; } catch (e) { }
      }
      try {
        const isMyTurn = (this.localPlayerId === this.currentPlayer);
        const overlay = document.getElementById('roletaOverlay');
        if (overlay) overlay.style.display = isMyTurn ? 'none' : 'flex';
        if (el) { if (isMyTurn) el.classList.remove('dim'); else el.classList.add('dim'); }
        if (api && typeof api.setSpinEnabled === 'function') {
          try { api.setSpinEnabled(isMyTurn); } catch (e) { }
          try { api.spinTo && api.resetWheel && api.resetWheel(); } catch (e) { }
        } else if (isIframe) {
          try { (el as HTMLIFrameElement).contentWindow!.postMessage({ type: 'setSpinEnabled', enabled: isMyTurn }, '*'); } catch (e) { }
          try { (el as HTMLIFrameElement).contentWindow!.postMessage({ type: 'resetWheel' }, '*'); } catch (e) { }
        }
  ...
  private setupRoletaForTurn() {
    try {
      ...
      // If component API available, call it. For iframe fallback to src/postMessage flow.
      if (api && typeof api.setSpinEnabled === 'function') {
        try { api.setSpinEnabled(this.localPlayerId === this.currentPlayer); } catch (e) { }
        try { if (typeof api.resetWheel === 'function') api.resetWheel(); } catch (e) { }
        try {
          const overlay = document.getElementById('roletaOverlay');
          if (overlay) overlay.style.display = (this.localPlayerId === this.currentPlayer) ? 'none' : 'flex';
          if (el) { if (this.localPlayerId !== this.currentPlayer) el.classList.add('dim'); else el.classList.remove('dim'); }
        ...
        const sendSetup = () => {
          try { iframe.contentWindow!.postMessage({ type: 'setSpinEnabled', enabled: (this.localPlayerId === this.currentPlayer) }, '*'); } catch (e) { }
